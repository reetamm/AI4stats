...
```{r eval = F}
install.pack ages(c("keras3","lime","evd"))
```
...
history <- GPD.model %>% fit(
  ...
  epochs = 100,
  verbose = 0,
  callback = list(checkpoint, early.stopping),
  validation_split = 0.2
)
...
plot(history)
test_loss_GPD2 <-  GPD.linear.model %>% evaluate(X_test.excess,
                                                 as.matrix(Y_test.excess),
                                                 batch_size = length(Y_test.excess))
...
plot(qexp(ps),qs,ylab="Fitted",xlab="Theoretical",main="Linear model",pch=20,asp=1)
abline(a=0,b=1,col="red")
...
Now, concatenate (join) the two models together. This will be our ouput later, i.e., `y_pred`. We also need to tell Keras what are the inputs and outputs:
```{r}
...
For the last model, we will make $\xi(\mathbf{x})$ constant. We achieve this by fixing the weights in the xi_model to zero, and allowing only the bias to be estimated.
```{r}
...
Now, copmile and train the final GPD model.
```{r}
GPD.model3 %>%
  compile(optimizer = "adam", loss = GPD_nll)
...
Here are our final mapped estimates for the last model. 
